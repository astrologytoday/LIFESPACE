...
struct MinorArcanaView: View {
    ...
    private var meaningSide: some View {
        Group {
            if deck.indices.contains(currentCardIndex) {
                VStack(spacing: 14) {
                    Text(deck[currentCardIndex].name)
    ...
    private func setupDeck() {
        deck = tarotCards.filter {
            $0.name.contains("Wands") ||
            $0.name.contains("Cups") ||
            $0.name.contains("Swords") ||
            $0.name.contains("Pentacles")
        }
