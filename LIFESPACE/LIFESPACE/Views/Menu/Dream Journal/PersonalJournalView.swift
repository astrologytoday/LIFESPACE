...
struct PersonalJournalView: View {
            ...
            VStack(spacing: 16) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        if personalJournalModel.canCreateEntry(for: todayDate) {
                            HStack {
                                Text(todayDate)
                                ...
                                Button(action: {
                                    navModel.push("JournalEntryView:new")
                                }) {
                                    Text("New Entry")
                        ...
                        ForEach(sortedEntriesDescending()) { entry in
                            HStack {
                                ...
                                Button(action: {
                                    navModel.push("JournalEntryView:\(entry.id.uuidString)")
                                }) {
                                    Text(entry.title)
                                        .font(.custom("IM FELL DW Pica", size: 20))
                                        .foregroundColor(.white)
                    ...
                    .padding(.bottom, 48) // so nothing hides behind the home button
                }
            }
    ...
    private func sortedEntriesDescending() -> [JournalEntry] {
        let formatter = DateFormatter()
        formatter.dateFormat = "MM/dd"
        return personalJournalModel.entries.sorted {
