...
struct FashionPicsView: View {
    @EnvironmentObject var navModel: NavigationModel

    struct FashionPhoto: Identifiable {
        let id = UUID()
        let image: UIImage
        let url: URL
        let date: Date?
    ...
    @State private var photos: [FashionPhoto] = []
    @State private var selectedPhoto: FashionPhoto? = nil
                            ...
                            ZStack(alignment: .topTrailing) {
                                ZStack(alignment: .bottomLeading) {
                                    ...
                                    VStack(alignment: .leading, spacing: 4) {
                                        if !p.outfitName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                                            Text(p.outfitName)
    ...
    private func photoViewer(for p: FashionPhoto) -> some View {
        ZStack {
            lifespaceGradient
            ...
            VStack {
                ...
                HStack(spacing: 10) {
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(spacing: 8) {
                            Text(p.outfitName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Untitled" : p.outfitName)
    ...
    private func editTitlePopup() -> some View {
        ZStack {
            ...
            VStack(spacing: 16) {
                ...
                TextField("Outfit name", text: $editedOutfitName)
    ...
    private func saveEditedTitle() {
        ...
        let cleanedTitle = editedOutfitName.trimmingCharacters(in: .whitespacesAndNewlines)

        FashionPicsMetadataStore.upsert(filename: filename, outfitName: cleanedTitle)
    ...
    private func loadSavedImages() {
        ...
        let meta = FashionPicsMetadataStore.load()
        ...
        do {
            ...
            print("Error loading fashion pics: \(error)")
        }
    }
    ...
    private func deleteImage(at index: Int) {
        ...
        do {
            ...
            FashionPicsMetadataStore.remove(filename: filename)
            photos.remove(at: index)
        } catch {
            print("Error deleting fashion pic: \(error)")
        }
    }
    ...
    private func parseDate(from filename: String) -> Date? {
        ...
        guard parts.count >= 4, parts[0] == "fashion" else {
            return nil
        }
