...
struct ADHDBuildingBlocksMatrixView: View {
                    ...
                    HStack(spacing: hSpacing) {
                        ForEach(pattern[r].indices, id: \.self) { c in
                            ...
                            if idx == -1 {
                                ...
                                ADHDMatrixCell(
                                    title: formatTitle(treatments[idx].title),
            ...
            if let i = selected {
                ZStack {
                    RoundedRectangle(cornerRadius: 20)
                        ...
                        .shadow(color: .yellow.opacity(0.25), radius: 10, y: 6)
...
private struct ADHDMatrixCell: View {
    let title: String
    let selected: Bool
    let size: CGSize
    let corner: CGFloat
    ...
    var body: some View {
        Button(action: action) {
            ZStack {
                ...
                RoundedRectangle(cornerRadius: corner)
                    .stroke(selected ? Color.yellow : Color.white.opacity(0.26), lineWidth: selected ? 3 : 1)
